...
class GitHubContentItem(BaseModel):
    ...
    size: Optional[int] = Field(
        None, description="Size of the file in bytes (if a file)"
    )
    ...
    html_url: Optional[str] = Field(
        None, description="HTML URL to view the content on GitHub"
    )
    token_estimate: Optional[int] = Field(
        0, description="Estimated token count for text content"
    )
...
class GitHubFileResponse(BaseModel):
    ...
    content: Optional[str] = Field(
        None, description="Text content of the file (if text)"
    )
