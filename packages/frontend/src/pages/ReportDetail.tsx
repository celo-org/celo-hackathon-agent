...
const ReportDetail = () => {
  ...
  const pollReportStatus = async (taskId: string) => {
    try {
      ...
      if (statusResponse.status.toUpperCase() === "COMPLETED") {
        try {
          ...
          // Clear the polling interval once we have the report
          if (pollingInterval) {
          ...
          // If we can't fetch the report but the task is completed,
          // there might be an issue with the report generation
          if (pollingCount > 5) {
            setIsLoading(false);
            if (pollingInterval) {
              clearInterval(pollingInterval);
            ...
            toast({
              ...
              description:
                "The analysis completed but the report couldn't be loaded",
              variant: "destructive",
            });
      ...
      else if (statusResponse.status.toUpperCase() === "FAILED") {
        setIsLoading(false);
        if (pollingInterval) {
          clearInterval(pollingInterval);
          setPollingInterval(null);
      ...
      // If task is completed but we didn't check yet for "completed" (lowercase)
      else if (
        statusResponse.status === "completed" ||
        (statusResponse.progress === 100 && statusResponse.completed_at)
      ) {
        try {
          const reportResponse = await api.get<ReportResponse>(
            `/reports/${taskId}`
      ...
      // If still in progress, update progress and continue polling
      else {
        // We'll continue polling via the interval
        console.log(
          "Task still in progress, status:",
      ...
      <Layout>
        <div className="analyzer-container py-10">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                {apiReport.analysis_type && (
                  <Badge variant="secondary" className="capitalize">
                    {apiReport.analysis_type === "fast"
                      ? "Fast Analysis"
                      : "Deep Analysis"}
                  </Badge>
                )}
          ...
          <Card>
            ...
            <CardContent>
              <div className="prose dark:prose-invert max-w-none prose-pre:bg-transparent prose-pre:p-0 prose-code:before:content-none prose-code:after:content-none">
                <EnhancedMarkdown>
                  {typeof apiReport.content === "string"
                    ? apiReport.content
                    : apiReport.content?.markdown || "No content available."}
                </EnhancedMarkdown>
              </div>
            </CardContent>
          </Card>
      ...
      <Layout>
        <div className="analyzer-container py-10">
          ...
          <Card>
            ...
            <CardContent>
              <div className="prose dark:prose-invert max-w-none prose-pre:bg-transparent prose-pre:p-0 prose-code:before:content-none prose-code:after:content-none">
                <EnhancedMarkdown>{mockReport.content}</EnhancedMarkdown>
              </div>
            </CardContent>
          </Card>
        </div>
      ...
      <Layout>
        <div className="analyzer-container py-10">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                <Badge variant="secondary" className="capitalize">
                  {mockDataReport.analysis_type === "deep"
                    ? "Deep Analysis"
                    : "Fast Analysis"}
                </Badge>
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="prose dark:prose-invert max-w-none prose-pre:bg-transparent prose-pre:p-0 prose-code:before:content-none prose-code:after:content-none">
                <EnhancedMarkdown>{mockDataReport.content}</EnhancedMarkdown>
              </div>
            </CardContent>
          </Card>
        </div>
